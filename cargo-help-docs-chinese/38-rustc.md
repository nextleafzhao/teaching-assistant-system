...
描述
       当前包（或提供的 -p 指定的包）的指定目标将与其所有依赖一起编译。
       指定的所有参数将传递给最终的编译器调用，而不是传递给任何依赖。
       请注意，编译器仍会无条件接收诸如 -L、--extern 和 --crate-type 等参数，
       而指定的参数将简单地添加到编译器调用中。

       有关 rustc 标志的文档，请参阅 <https://doc.rust-lang.org/rustc/index.html>。

       此命令要求在提供额外参数时仅编译一个目标。
       如果当前包有多个可用目标，必须使用 --lib、--bin 等过滤器来选择编译哪个目标。

       要向 Cargo 生成的所有编译器进程传递标志，请使用 RUSTFLAGS 环境变量
...
选项
   ...
   目标选择
       当未给出目标选择选项时，cargo rustc 将构建所选包的所有二进制和库目标。
       ...
       --bin name…
           构建指定的二进制文件。此标志可以多次指定，并支持常见的 Unix 通配符模式。

       --bins
           构建所有二进制目标。

       --example name…
           构建指定的示例。此标志可以多次指定，并支持常见的 Unix 通配符模式。

       --examples
           构建所有示例目标。

       --test name…
           构建指定的集成测试。此标志可以多次指定，并支持常见的 Unix 通配符模式。

       --tests
           构建所有设置了 test = true 清单标志的目标。默认情况下，
       ...
       --bench name…
           构建指定的基准测试。此标志可以多次指定，并支持常见的 Unix 通配符模式。

       --benches
           构建所有设置了 bench = true 清单标志的目标。默认情况下，
   ...
   编译选项
       ...
       -r, --release
           使用 release profile 构建优化的构件。
           另请参阅 --profile 选项以按名称选择特定 profile。

       --profile name
           使用给定的 profile 构建。

           rustc 子命令将以特殊行为处理以下命名的 profile：

           o  check — 与 cargo-check(1) 命令使用 dev profile 的构建方式相同。

           o  test — 与 cargo-test(1) 命令的构建方式相同，
              启用测试模式构建，这将启用测试并启用 test cfg 选项。
              有关更多详细信息，请参阅 rustc 测试
              <https://doc.rust-lang.org/rustc/tests/index.html>。

           o  bench — 与 cargo-bench(1) 命令的构建方式相同，类似于 test profile。
       ...
       --crate-type crate-type
           为给定的 crate 类型构建。此标志接受 1 个或多个 crate 类型的逗号分隔列表，
           其中允许的值与配置 Cargo 目标的清单中的 crate-type 字段相同。
           有关可能的值，请参阅 crate-type 字段
           <https://doc.rust-lang.org/cargo/reference/cargo-targets.html#the-crate-type-field>。

           如果清单包含列表，且提供了 --crate-type，则命令行参数值将覆盖清单中的值。

           此标志仅在构建 lib 或示例库目标时有效。
   ...
   杂项选项
       ...
       --keep-going
           ...
           cargo rustc -j1 可能会也可能不会构建 works（取决于 Cargo 选择先运行哪个构建），
           而 cargo rustc -j1 --keep-going 肯定会运行两个构建，即使先运行的那个失败。
...
示例
       1. 检查你的包（不包括依赖）是否使用了不安全代码：

              cargo rustc --lib -- -D unsafe-code

       2. 在 nightly 编译器上尝试实验性标志，例如打印每个类型的大小：
