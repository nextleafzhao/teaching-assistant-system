...
选项
   包选择
       ...
       -p spec…, --package spec…
           仅构建指定的包。有关 SPEC 格式，请参阅 cargo-pkgid(1)。
   ...
   目标选择
       当未给出目标选择选项时，cargo build 将构建所选包的所有二进制和库目标。
   ...
   编译选项
       ...
       -r, --release
           使用 release profile 构建优化的构件。另请参阅 --profile 选项以按名称选择特定 profile。

       --profile name
           使用给定的 profile 构建。有关 profile 的更多详细信息，请参阅参考文档
   ...
   杂项选项
       ...
       --keep-going
           ...
           cargo build -j1 可能会也可能不会构建 works（取决于 Cargo 选择先运行哪个构建），
           而 cargo build -j1 --keep-going 肯定会运行两个构建，即使先运行的那个失败。
