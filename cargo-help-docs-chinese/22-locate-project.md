...
描述
       此命令将向 stdout 打印一个 JSON 对象，包含清单的完整路径。
       通过从当前工作目录向上搜索名为 Cargo.toml 的文件来找到清单。

       如果项目恰好是工作区的一部分，则输出项目的清单，而非工作区根目录。
       这可以通过 --workspace 标志覆盖。
       通过进一步向上遍历或使用 package.workspace 字段（在找到工作区成员的清单后）来找到根工作区。

选项
       --workspace
           定位工作区根目录的 Cargo.toml，而非当前工作区成员。

   显示选项
       --message-format fmt
           打印项目位置的表示形式。有效值：

           o  json (默认): JSON 对象，路径在 "root" 键下。
