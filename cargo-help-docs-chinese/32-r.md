...
描述
       ...
       双破折号 (--) 之后的所有参数都会传递给要运行的二进制文件。
       如果你同时向 Cargo 和二进制传递参数，-- 之后的参数传递给二进制文件，
       之前的参数传递给 Cargo。

       与 cargo-test(1) 和 cargo-bench(1) 不同，cargo run 将执行的二进制文件
       的工作目录设置为当前工作目录，与直接在 shell 中执行相同。
...
选项
   ...
   目标选择
       当未给出目标选择选项时，cargo run 将运行二进制目标。
       如果有多个二进制目标，你必须传递目标标志来选择一个。
       或者，可以在 Cargo.toml 的 [package] 部分指定 default-run 字段，
       以选择默认运行的二进制文件名称。
   ...
   杂项选项
       ...
       --keep-going
           ...
           cargo run -j1 可能会也可能不会构建 works（取决于 Cargo 选择先运行哪个构建），
           而 cargo run -j1 --keep-going 肯定会运行两个构建，即使先运行的那个失败。
