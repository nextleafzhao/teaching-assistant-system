...
名称
       cargo-rustdoc — 构建包的文档，使用指定的自定义标志

概要
       cargo rustdoc [选项] [-- 参数]

描述
       当前包（或提供的 -p 指定的包）的指定目标将使用指定的参数生成文档，
       这些参数将传递给最终的 rustdoc 调用。依赖不会作为此命令的一部分生成文档。
       请注意，rustdoc 仍会无条件接收诸如 -L、--extern 和 --crate-type 等参数，
       而指定的参数将简单地添加到 rustdoc 调用中。

       有关 rustdoc 标志的文档，请参阅 <https://doc.rust-lang.org/rustdoc/index.html>。

       此命令要求在提供额外参数时仅编译一个目标。
       如果当前包有多个可用目标，必须使用 --lib、--bin 等过滤器来选择编译哪个目标。

       要向 Cargo 生成的所有 rustdoc 进程传递标志，请使用 RUSTDOCFLAGS 环境变量
       <https://doc.rust-lang.org/cargo/reference/environment-variables.html> 或
       build.rustdocflags 配置值
       <https://doc.rust-lang.org/cargo/reference/config.html>。

选项
   文档选项
       --open
           构建文档后在浏览器中打开它们。除非你在 BROWSER 环境变量中定义了另一个浏览器，
           或使用 doc.browser <https://doc.rust-lang.org/cargo/reference/config.html#docbrowser> 配置选项，
           否则将使用你的默认浏览器。

   包选择
       默认情况下，选择当前工作目录中的包。
       可以使用 -p 标志在工作区中选择不同的包。

       -p spec, --package spec
           要生成文档的包。有关 SPEC 格式，请参阅 cargo-pkgid(1)。

   目标选择
       当未给出目标选择选项时，cargo rustdoc 将为所选包的所有二进制和库目标生成文档。
       如果二进制文件的名称与 lib 目标相同，则会被跳过。
       如果二进制文件缺少所需的特性，则会被跳过。

       传递目标选择标志将仅为指定的目标生成文档。
       ...
       --bin name…
           为指定的二进制文件生成文档。此标志可以多次指定，并支持常见的 Unix 通配符模式。

       --bins
           为所有二进制目标生成文档。

       --example name…
           为指定的示例生成文档。此标志可以多次指定，并支持常见的 Unix 通配符模式。

       --examples
           为所有示例目标生成文档。

       --test name…
           为指定的集成测试生成文档。此标志可以多次指定，并支持常见的 Unix 通配符模式。

       --tests
           为所有设置了 test = true 清单标志的目标生成文档。默认情况下，
       ...
       --bench name…
           为指定的基准测试生成文档。此标志可以多次指定，并支持常见的 Unix 通配符模式。

       --benches
           为所有设置了 bench = true 清单标志的目标生成文档。默认情况下，
       ...
       --all-targets
           为所有目标生成文档。这等同于指定 --lib --bins --tests --benches --examples。
   ...
   杂项选项
       ...
       --keep-going
           ...
           cargo rustdoc -j1 可能会也可能不会构建 works（取决于 Cargo 选择先运行哪个构建），
           而 cargo rustdoc -j1 --keep-going 肯定会运行两个构建，即使先运行的那个失败。

       --output-format
           文档生成的输出类型。有效值：

           o  html (默认): 以 HTML 格式生成文档。

           o  json: 以实验性的 JSON 格式生成文档
              <https://doc.rust-lang.org/nightly/nightly-rustc/rustdoc_json_types>。

           此选项仅在 nightly 通道
           <https://doc.rust-lang.org/book/appendix-07-nightly-rust.html> 上可用，
           并且需要 -Z unstable-options 标志来启用。
