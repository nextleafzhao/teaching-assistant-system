...
export const MOCK_PLANS: TeachingPlan[] = [
  {
    ...
    name: '高二数学冲刺计划',
    steps: [
      { id: 1, description: '导数基础', knowledgePointIds: [1, 2], estimatedDays: 3, completed: true, completedDate: '2026-04-05' },
      { id: 2, description: '导数应用', knowledgePointIds: [3, 4], estimatedDays: 5, completed: false },
      { id: 3, description: '综合练习', knowledgePointIds: [5], estimatedDays: 4, completed: false },
    ],
  },
  {
    id: 2,
    name: '高三一轮复习',
    steps: [
      { id: 4, description: '集合与逻辑', knowledgePointIds: [6], estimatedDays: 2, completed: true, completedDate: '2026-03-28' },
      { id: 5, description: '函数与方程', knowledgePointIds: [7, 8], estimatedDays: 7, completed: false },
      { id: 6, description: '三角函数', knowledgePointIds: [9, 10], estimatedDays: 5, completed: false },
    ],
  },
]
...
export const MOCK_GRADE_RECORDS: GradeRecord[] = [
  { id: 1, examId: 1, studentId: 1, studentName: '张三', score: 125, isAbsent: false, status: 'submitted' },
  { id: 2, examId: 1, studentId: 2, studentName: '李四', score: 98, isAbsent: false, status: 'submitted' },
  { id: 3, examId: 1, studentId: 3, studentName: '王五', score: 135, isAbsent: false, status: 'submitted' },
]
...
export async function createMemoTodo(item: Omit<MemoTodo, 'id' | 'createdAt'>): Promise<MemoTodo> {
  const newItem: MemoTodo = { ...item, id: Date.now(), createdAt: new Date().toISOString().split('T')[0] }
...
// 检查课程冲突
export async function checkCourseConflict(studentId: number, date: string, startTime: string, endTime: string): Promise<boolean> {
  const hasConflict = MOCK_COURSES.some(c => 
