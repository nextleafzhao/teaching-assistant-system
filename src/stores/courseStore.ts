  ...
  actions: {
    async fetchCourses(weekStart: string) {
      ...
      try {
        ...
        console.log('[CourseStore] fetchCourses - success, count:', this.courses.length)
      } catch (err) {
    ...
    async checkConflict(studentId: number, date: string, startTime: string, endTime: string): Promise<boolean> {
      console.log('[CourseStore] checkConflict - student:', studentId, date, startTime, endTime)
      const result = await api.checkCourseConflict(studentId, date, startTime, endTime)
